"""Real, Appium-backed Instamart adapter — Phase 8 (search) + Phase 14
(cart/checkout).

Implements the SAME ``StoreAdapter`` protocol as ``InstamartAdapter`` (Phase
...
``app.adapters._appium_order`` (Phase 14). Both return typed failure
...
from app.adapters.instamart.locators import LOCATORS
...
from app.automation.waits import DEFAULT_TIMEOUT_SECONDS
from app.core.config import Settings
from app.domain.raw_product_result import RawProductResult

STORE_ID = "instamart"

_logger = logging.getLogger("app.adapters.instamart.appium")


class InstamartAppiumAdapter:
    """Real Appium-backed Instamart integration.

    NOTE: ``app/adapters/instamart/locators.py`` currently contains
