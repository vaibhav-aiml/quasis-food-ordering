...
the Phase 15 LangGraph workflow (``app.graph.workflow.build_graph``).
...
3. Resume a paused thread with an explicit, human-supplied approval
   ...
   (``POST /{id}/approval``) — reusing ``app.domain.approval.
...
4. Let the caller pick a specific ranked candidate for a product before
   ...
   ``app.processing.recommendation_selection.select_best_store`` and
...
- ``ready_for_payment`` is only ever reported ``True`` when
  ...
  ``app.graph.nodes.order_execution`` for the matching graph-side fix
...
from app.agents.recommendation_agent import RecommendationGenerator, RecommendationResult
from app.core.dependencies import get_recommendation_generator, get_shopping_graph_dependency
from app.domain.approval import ApprovalSubmission
...
class RequestCreate(BaseModel):
    ...
    selected_indices: dict[str, int] | None = Field(
        ...
        description=(
            ...
            "see app.processing.recommendation_selection.select_best_store. "
...
def submit_approval(
    ...
    ``submission`` is ``app.domain.approval.ApprovalSubmission`` itself
