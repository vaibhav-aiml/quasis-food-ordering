"""Tests for app.adapters.locators.

These test SHAPE only — not locator correctness, which cannot be
verified without a real device (see Phase 8 docs, section 6).
"""

from app.adapters.blinkit.locators import LOCATORS as BLINKIT_LOCATORS
from app.adapters.instamart.locators import LOCATORS as INSTAMART_LOCATORS
...
from app.adapters.zepto.locators import LOCATORS as ZEPTO_LOCATORS
...
def test_checkout_locators_has_no_payment_confirmation_field() -> None:
    ...
    just by convention. See app.adapters._appium_order's module docstring.
    """
